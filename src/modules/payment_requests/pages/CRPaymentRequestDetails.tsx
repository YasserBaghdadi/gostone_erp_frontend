...
import { ArrowLeft, PackageOpen, Loader2, AlertCircle, FileText, Calendar, User, BadgeDollarSign, CheckCircle } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Separator } from "@/components/ui/separator";
import { toast } from "sonner";
import { useCRPaymentRequestDetails, useMarkCRPaymentDone } from "@/hooks/usePaymentRequests";
...
export default function CRPaymentRequestDetails() {
  const { id } = useParams();
  const navigate = useNavigate();
  const { data, isLoading, isError } = useCRPaymentRequestDetails(id || "");
  const { data: returnDetail } = useCustomerReturnDetails(
  ...
  const returnLinkText = customerReturnLinkLabel(
    data.customer_return,
    returnDetail,
  );
  const returnLinkTitle = customerReturnLinkTitle(
    data.customer_return,
    returnDetail,
  );
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold tracking-tight">طلب دفع مرتجع #{data.id}</h1>
            ...
            <p className="text-muted-foreground mt-1 flex items-center gap-2 min-w-0">
              ...
              <Link
                to={`/customer-returns/${data.customer_return}`}
          ...
          <Card className="shadow-sm border-none ring-1 ring-border/50">
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between gap-2 min-w-0">
                <span className="text-muted-foreground flex items-center gap-2 shrink-0"><PackageOpen className="h-4 w-4" /> المرتجع</span>
                <Link
                  to={`/customer-returns/${data.customer_return}`}
              ...
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground flex items-center gap-2"><User className="h-4 w-4" /> العميل</span>
                <span className="text-sm font-medium">{formatCrPaymentCustomerLabel(data, returnDetail ?? null)}</span>
              </div>
              <div className="flex items-center justify-between">
